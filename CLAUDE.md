...
- **In progress:** —
- **Try it now (no setup):** `pnpm install && pnpm dev` → http://localhost:3000/studio opens the editor in **demo mode** (edits saved to localStorage, Publish disabled); http://localhost:3000/demo shows the public render. Demo mode activates only while env vars are absent.
...
- 2026-06-10 — **Local demo mode**: with no Supabase env, `/studio` runs the real editor on the starter config (autosave → localStorage, Publish disabled, "Demo mode" badge) and `/demo` statically renders the public output. Real auth/DB flow is untouched and takes over automatically once `.env.local` exists. — Owner wanted to try the product locally before doing the Supabase setup.
